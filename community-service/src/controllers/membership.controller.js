...
export class MembershipController {
	...
	static async joinCommunity(req, res) {
		try {
			...
			const membership = await MembershipService.joinCommunity(
				id,
				userId,
				status
			)
	...
	/**
	 * Update membership (status, role, flair, etc.)
	 * Body can contain: { status: "approved" | "banned", role: "member" | "moderator" | "admin", flair: "..." }
	 */
	...
	static async deleteMembership(req, res) {
		try {
			const { membershipId } = req.params
			const deleted = await MembershipService.deleteMembershipById(membershipId)
			res.status(200).json(deleted)
		} catch (error) {
			res.status(400).json({ error: error.message })
		}
