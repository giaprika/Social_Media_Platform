...
export class NotificationConsumer {
  ...
  static async handlePostCreated(data) {
    ...
    const followers = await NotificationService.getFollowersOfUser(
      data.user_id
    );
    ...
    const postPreview = data.post_title
      ? data.post_title.substring(0, 50) +
    ...
    await NotificationService.createNotificationToMultipleUsers({
      ...
      body_template: `${authorName} đã đăng bài viết mới${
        postPreview ? ': "' + postPreview + '"' : ""
      }`,
      link_url: `/app/posts/${data.post_id}`,
    });
    console.log(
      `Post created notification sent to ${followerIds.length} followers of ${data.user_id}`
    );
  }
  ...
  static async handlePostLiked(data) {
    ...
    const existing = await NotificationRepository.findAggregatedNotification(
      data.user_id,
      "post_liked",
      postId
    );
    ...
    console.log(
      `Aggregated like notification sent to user ${data.user_id} for post ${postId}`
    );
  }
  ...
  static async handlePostCommented(data) {
    ...
    const postId =
      data.post_id || data.link_url?.split("/posts/")[1]?.split("#")[0];
    const commenterName = data.commenter_username || "Ai đó";
    const commentPreview = data.comment_content
      ? data.comment_content.substring(0, 30) +
        (data.comment_content.length > 30 ? "..." : "")
      : "";
    ...
    console.log(
      `Aggregated comment notification sent to user ${data.user_id} for post ${postId}`
    );
  }
