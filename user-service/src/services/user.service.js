...
export class UserService {
  ...
  static async unfollowUser(userId, targetId) {
    ...
    const result = await UserRepository.unfollowUser(userId, targetId);

    // Publish "user.unfollowed" event
    try {
      const unfollower = await UserRepository.findUserById(userId);
      if (unfollower) {
        const eventData = {
          user_id: targetId,
          unfollower_id: userId,
          unfollower_username: unfollower.username,
          timestamp: new Date().toISOString(),
        };

        await RabbitMQProducer.publish("user.unfollowed", eventData);
      }
    } catch (err) {
      console.error("[UserService] Failed to publish unfollow event:", err);
    }
