...
export class RelationshipsService {
	static async createRelationship({ user_id, target_id, type }) {
		...
		if (type === 'follow') {
			...
			const result = await RelationshipsRepository.insertRelationship({
				id,
				user_id,
				target_id,
				type: 'follow',
			...
			try {
				const { RabbitMQProducer } = await import('./rabbitmq.producer.js')
				await RabbitMQProducer.connect()
				await RabbitMQProducer.publish('user.followed', {
					user_id: target_id, // người bị follow sẽ nhận thông báo
					follower_id: user_id,
					follower_username: undefined, // Có thể lấy thêm username nếu cần
					title_template: 'Bạn có người theo dõi mới!',
					body_template: `Ai đó vừa theo dõi bạn!`,
					link_url: `/profile/${user_id}`,
				})
			} catch (err) {
				console.error(
					'[Notification] Failed to publish follow event',
					err.message
				)
			}
