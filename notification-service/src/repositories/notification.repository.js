...
export class NotificationRepository {
  static async findNotificationByUserId(user_id) {
    const result = await db.query(
      `SELECT id, user_id, title_template, body_template, reference_id, 
  ...
  static async createNotificationToMultipleUsers(
    user_ids,
    title_template,
    body_template,
    link_url
  ...
  static async findAggregatedNotification(user_id, reference_id) {
    const result = await db.query(
      `SELECT id, user_id, title_template, body_template, reference_id, 
  ...
  static async upsertAggregatedNotification({
    user_id,
    reference_id,
    title_template,
    body_template,
    ...
    const existing = await this.findAggregatedNotification(
      user_id,
      reference_id
    );
    ...
    if (existing) {
      ...
      const result = await db.query(
        `UPDATE notifications 
         SET actors_count = $1, 
             ...
             updated_at = NOW()
         WHERE id = $5
         RETURNING id, user_id, title_template, body_template, reference_id, 
                   actors_count, last_actor_id, last_actor_name, is_readed, created_at, updated_at, link_url`,
      ...
      const result = await db.query(
        `INSERT INTO notifications (id, user_id, title_template, body_template, reference_id, actors_count, last_actor_id, last_actor_name, link_url) 
